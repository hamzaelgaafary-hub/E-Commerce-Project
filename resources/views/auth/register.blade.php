<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <div class="mt-2 font-semibold text-gray-700 dark:text-gray-300">
                @foreach ($roles as $role)
                    <label for="role_{{ $role->id }}" class="inline-flex items-center">
                        <input id="role_{{ $role->id }}" type="radio" name="role_id" value="{{ $role->id }}" @if(old('role_id') == $role->id) checked @endif>
                        <span class="ml-2">{{ __(ucfirst($role->name)) }}</span>
                    </label>
                    <br>
                @endforeach
            </div>
